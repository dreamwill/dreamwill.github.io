...
4. 创建 playbook[^1]
    ...
    我在这里使用 vim 创建并编辑名为 `setup-macos.yml` 的 playbook，你可以使用其他的文本编辑器。

    ```bash
    vim setup-macos.yml
    ```

    ```yaml title="setup-macos.yml"
    ---
    - hosts: localhost
      connection: local
      tasks:
...
[^1]: playbook 是 Ansible 世界中的脚本，用于描述你希望设备最终达到的状态。
[^2]: Tart 是 macOS 上虚拟 macOS 的软件，且要求宿主机搭载 Apple M1/2/3/4 系列芯片。