...
```
hadoop 命令不支持 FreeIPA 安装后的`default_ccache_name`配置项（KCM），所以我通过改变 hdfs 用户的环境变量，使得 hdfs 用户使用 kinit 命令进行 Kerberos 身份认证时，使用/tmp/krb5cc_%{uid}的 credential cache。
